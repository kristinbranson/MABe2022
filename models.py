...
class OutConv(nn.Module):
    def __init__(self, in_channels, out_channels, n_collapse):
        super(OutConv, self).__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=(1,n_collapse))
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_categories, n_targets, nchannels_inc=8, bilinear=True):
        super(UNet, self).__init__()
        self.n_channels = n_channels
        self.n_categories = n_categories
        self.n_targets = n_targets
        self.bilinear = bilinear
        self.nchannels_inc = nchannels_inc
        ...
        self.layer_outc = OutConv(nchannels_up3 // factor, self.n_categories, self.n_targets)

    def forward(self, x, verbose=False):
        if verbose: print('input: shape = '+str(x.shape))
        x1 = self.layer_inc(x)
